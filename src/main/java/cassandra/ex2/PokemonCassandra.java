...
public class PokemonCassandra {
    ...
    public static void main(String[] args) {
        ...
        try(CqlSession session = CqlSession.builder()
            ...
            CreateTableWithOptions create = createTable("big_data_pokemon", "pokemon")
                    ...
                    .withPartitionKey("type_2", DataTypes.TEXT)
                    ...
                    .withClusteringOrder("generation", ClusteringOrder.DESC)
            ...
            System.out.println("Pokemons with type: 'Water' and type2: 'Flying'");
            // directly query
            PagingIterable<Pokemon> waterFlying = dao.findByTypeAndType2("Water", "Flying");
            for (Pokemon p : waterFlying) {
                System.out.println(p.getName() + " (" + p.getGeneration() + ")");
            }

            // show count of pokemons for each type-type_2 combination
            printTypeCombinationDistribution(dao);
    ...
    public static void printTypeCombinationDistribution(PokemonDao dao) {
        Map<String, Long> typeCombinationCounts = new HashMap<>();

        for (Pokemon p : dao.getAllPokemons()) {
            String key = p.getType() + "," + (p.getType2() == null || p.getType2().isEmpty() ? "None" : p.getType2());
            typeCombinationCounts.put(key, typeCombinationCounts.getOrDefault(key, 0L) + 1);
        }

        System.out.println("\nPokémon Type Combination Distribution");
        typeCombinationCounts.entrySet().stream()
