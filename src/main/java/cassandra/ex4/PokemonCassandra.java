...
public class PokemonCassandra {
    ...
    public static void main(String[] args) {
        ...
        try(CqlSession session = CqlSession.builder()
            ...
            List<Pokemon> legendaryList = new ArrayList<>();
            dao.getLegendary().forEach(legendaryList::add);
            printAverageStatsByType(legendaryList);
    ...
    private static void printAverageStatsByType(List<Pokemon> pokemons) {
        Map<String, List<Pokemon>> grouped = new HashMap<>();

        for (Pokemon p : pokemons) {
            grouped.computeIfAbsent(p.getType(), k -> new ArrayList<>()).add(p);
        }

        for (Map.Entry<String, List<Pokemon>> entry : grouped.entrySet()) {
            String type = entry.getKey();
            List<Pokemon> group = entry.getValue();

            int count = group.size();
            double averageTotal = group.stream().mapToInt(Pokemon::getTotal).average().orElse(0);
            double averageHP = group.stream().mapToInt(Pokemon::getHp).average().orElse(0);
            double averageAttack = group.stream().mapToInt(Pokemon::getAttack).average().orElse(0);
            double avgDefense = group.stream().mapToInt(Pokemon::getDefense).average().orElse(0);
            double avgSpeedAttack = group.stream().mapToInt(Pokemon::getSpeedAttack).average().orElse(0);
            double avgSpeedDefense = group.stream().mapToInt(Pokemon::getSpeedDefence).average().orElse(0);
            double avgSpeed = group.stream().mapToInt(Pokemon::getSpeed).average().orElse(0);

            System.out.printf("\n\nType: %-12s <- %-2d Legendary Pokemons\n"+
                    "\t Avg Total: %.2f%n \t Avg HP: %.2f%n " +
                    "\t Avg Attack: %.2f%n \t Avg Defense: %.2f%n \t Avg Speed Attack: %.2f%n \t Avg Speed Defense: %.2f%n" +
                            "\t Avg Speed: %.2f%n",
                    type, count, averageTotal, averageHP, averageAttack, avgDefense,
                    avgSpeedAttack, avgSpeedDefense, avgSpeed);
        }
    }
